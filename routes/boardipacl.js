if(hostconfig.namuwiki_exclusive) router.post(/^\/admin\/boardipacl\/remove$/, async(req, res) => {
	...
	var dbdata = await curs.execute("select cidr from boardipacl where cidr = ?", [req.body['ip']]);
	if(!dbdata.length) return res.status(400).send(await showError(req, 'invalid_value'));
	await curs.execute("delete from boardipacl where cidr = ?", [req.body['ip']]);
	return res.redirect('/admin/boardipacl');
});

if(hostconfig.namuwiki_exclusive) router.all(/^\/admin\/boardipacl$/, async(req, res, next) => {
	if(!['POST', 'GET'].includes(req.method)) return next();
	if(!hasperm(req, 'ipacl')) return res.status(403).send(await showError(req, 'permission'));
	const { from, until } = req.query;
	var error = null;
	
	await curs.execute("delete from boardipacl where not expiration = '0' and ? > cast(expiration as integer)", [Number(getTime())]);
	var ld   = await curs.execute("select cidr from boardipacl order by cidr desc limit 1");
	var fd   = await curs.execute("select cidr from boardipacl order by cidr asc limit 1");
	var data = await curs.execute("select cidr, expiration, note, date from boardipacl " + (from ? "where cidr > ?" : (until ? "where cidr < ?" : "")) + " order by cidr " + (until ? 'desc' : 'asc') + " limit 50", (from || until ? [from || until] : []));
	if(until) data = data.reverse();
	try {
		var navbtns = navbtnss(fd[0].cidr, ld[0].cidr, data[0].cidr, data[data.length-1].cidr, '/admin/boardipacl');
	} catch(e) {
		...
		content += `
			<tr>
				...
				<td class=text-center>
					<form method=post onsubmit="return confirm('정말로?');" action="/admin/boardipacl/remove">
	...
	if(req.method == 'POST') {
		...
		if(!ip.match(/^([01]?[0-9][0-9]?|2[0-4][0-9]|25[0-5])[.]([01]?[0-9][0-9]?|2[0-4][0-9]|25[0-5])[.]([01]?[0-9][0-9]?|2[0-4][0-9]|25[0-5])[.]([01]?[0-9][0-9]?|2[0-4][0-9]|25[0-5])\/([1-9]|[12][0-9]|3[0-2])$/)) error = true, content = alertBalloon(fetchErrorString('invalid_cidr'), 'danger', true, 'fade in') + content;
			...
			if(data.length) content = (error = err('alert', { code: 'ipacl_already_exists' })) + content;
				...
				await curs.execute("insert into boardipacl (cidr, expiration, note, date) values (?, ?, ?, ?)", [ip, expiration, note, date]);

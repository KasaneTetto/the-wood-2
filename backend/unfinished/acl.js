function ACL(req, res, title) {
	const doc = processTitle(title);
	if(['특수기능', '투표', '토론'].includes(doc.namespace) || !doc.title) return await showError(req, '문서 이름이 올바르지 않습니다.', 1);
	if(ver('4.2.0')) {
		...
		async function tbody(type, isns, edit) {
			...
			for(var row of data) {
				ret.push({
					id: i++,
					condition: `${row.conditiontype}:${row.condition}`,
					action: row.action,

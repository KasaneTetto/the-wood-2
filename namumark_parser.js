...
for(let item in functions) global[item] = functions[item];
...
function parseTable(content) {
	...
	for(let _tr of (data.match(/^(<tr norender><td>(((?!<\/td><\/tr>($|\n))[\s\S])*)<\/td><\/tr>)$/gim) || [])) {
		let tr = _tr.match(/^(<tr norender><td>(((?!<\/td><\/tr>($|\n))[\s\S])*)<\/td><\/tr>)$/im)[0];
		
		if (  // 표의 시작이라면(위에 || 문법 없음)
			(!((befrow = (datarows[datarows.findIndex(s => s == tr.split('\n')[0]) - 1] || '')).match(/^(<tr><td>(((?!<\/td><\/tr>($|\n))[\s\S])*)<\/td><\/tr>)$/im))) &&  // 이전 줄이 표가 아니면
			(!(befrow.match(/^(\|(((?!\|).)+)\|(((?!\|\|($|\n))[\s\S])*)\|\|)$/im)))  // 캡션도 아니면
		) {
			let fulloptions = (tr.replace(/&lt;((?!table).)*&gt;/g, '').match(/^<tr norender><td>((&lt;([a-z0-9 ]+)=(((?!&gt;).)+)&gt;)+)/i) || ['', ''])[1];
			let ts = '', trs = '';
			
			let alop, align = ((alop = (fulloptions.match(/&lt;table\s*align=(left|center|right)&gt;/))) || ['', 'left'])[1];
			if(alop) data = data.replace(tr, tr = tr.replace(alop[0], ''));
			
			let wiop, width = ((wiop = (fulloptions.match(/&lt;table\s*width=((\d+)(px|%|))&gt;/))) || ['', ''])[1];
			if(wiop) {
			...
			let clop, color = ((clop = (fulloptions.match(/&lt;table\s*color=((#[a-fA-F0-9]{3,6})|([a-zA-Z]+))(([,]((#[a-fA-F0-9]{3,6})|([a-zA-Z]+)))|)&gt;/))) || ['', ''])[1];
			if(clop) {
				data = data.replace(tr, tr = tr.replace(clop[0], ''));
				trs += 'color: ' + color + '; ';
			}
			
			let bgop, bgcolor = ((bgop = (fulloptions.match(/&lt;table\s*bgcolor=((#[a-fA-F0-9]{3,6})|([a-zA-Z]+))(([,]((#[a-fA-F0-9]{3,6})|([a-zA-Z]+)))|)&gt;/))) || ['', ''])[1];
			if(bgop) {
				data = data.replace(tr, tr = tr.replace(bgop[0], ''));
				trs += 'background-color: ' + bgcolor + '; ';
			}
			
			let brop, border = ((brop = (fulloptions.match(/&lt;table\s*bordercolor=((#[a-fA-F0-9]{3,6})|([a-zA-Z]+))(([,]((#[a-fA-F0-9]{3,6})|([a-zA-Z]+)))|)&gt;/))) || ['', ''])[1];
			if(brop) {
	...
	for(let _tr of (data.match(/^(\|(((?!\|).)+)\|(((?!\|\|($|\n))[\s\S])*)\|\|)$/gim) || [])) {
		...
		let fulloptions = (ntr.replace(/&lt;((?!table).)*&gt;/g, '').match(/^<tr><td>((&lt;([a-z0-9 ]+)=(((?!&gt;).)+)&gt;)+)/i) || ['', ''])[1];
	...
	for(let _tr of (data.match(/^<tr>(((?!<\/tr>).)*)<\/tr>$/gim) || [])) {
		let tr = _tr.match(/^<tr>(((?!<\/tr>).)*)<\/tr>$/im)[1], ntr = tr;
		let rowstyle = '';
		
		for(let td of (tr.match(/<td>(((?!<\/td>).)*)<\/td>/g) || [])) {
			let text = (td.match(/<td>(((?!<\/td>).)*)<\/td>/) || ['', ''])[1], ot = text, ntd = td;
			let notx = text.replace(/^((&lt;([a-z0-9():\| -]+)((=(((?!&gt;).)+))*)&gt;)+)/i, '');
			let attr = '', tds = '', cs = '', rs = '';
			
			let fulloptions = (td.replace(/(&lt;table([a-z0-9 ]+)=(((?!&gt;).)+)&gt;)/g, '').match(/^<td>((&lt;([a-z0-9():\|\^ -]+)((=(((?!&gt;).)+))*)&gt;)+)/i) || ['', ''])[1];
			...
			// 너비
			let width = (fulloptions.match(/&lt;width=((\d+)(px|%|))&gt;/) || ['', ''])[1];
			if(width) {
				tds += 'width: ' + width + '; ';
			...
			// 높이
			let height = (fulloptions.match(/&lt;height=((\d+)(px|%|))&gt;/) || ['', ''])[1];
			if(height) {
				tds += 'height: ' + height + '; ';
			...
			// 가로 합치기
			let colspan = (fulloptions.match(/&lt;[-](\d+)&gt;/) || ['', ''])[1];
			if(colspan) {
				cs = colspan;
				ntd = ntd.replace(/&lt;[-](\d+)&gt;/, '');
			}
			...
			// 세로 합치기 & 정렬
			let rowopt = (fulloptions.match(/&lt;([^]|[v]|)[|](\d+)&gt;/) || ['', '', '']);
			if(rowopt[2]) {
				rs = rowopt[2];
				switch(rowopt[1]) {
					case '^':
			...
			// 셀 배경색
			let bgcolor = (fulloptions.match(/&lt;((#([a-fA-F0-9]{3,6}))|([a-zA-Z]+))(([,]((#([a-fA-F0-9]{3,6}))|([a-zA-Z]+)))|)&gt;/) || ['', ''])[1];
			if(bgcolor) {
			...
			// 셀 배경색 2
			bgcolor = (fulloptions.match(/&lt;bgcolor=((#[a-fA-F0-9]{3,6})|([a-zA-Z]+))(([,]((#[a-fA-F0-9]{3,6})|([a-zA-Z]+)))|)&gt;/) || ['', ''])[1];
			if(bgcolor) {
				tds += 'background-color: ' + bgcolor + '; ';
			...
			// 글자색
			let color = (fulloptions.match(/&lt;color=((#[a-fA-F0-9]{3,6})|([a-zA-Z]+))(([,]((#[a-fA-F0-9]{3,6})|([a-zA-Z]+)))|)&gt;/) || ['', ''])[1];
			if(color) {
				tds += 'color: ' + color + '; ';
			...
			// 줄 배경색
			let rowbgcolor = (fulloptions.match(/&lt;rowbgcolor=((#[a-fA-F0-9]{3,6})|([a-zA-Z]+))(([,]((#[a-fA-F0-9]{3,6})|([a-zA-Z]+)))|)&gt;/) || ['', ''])[1];
			if(rowbgcolor) {
			...
			// 줄 글자색
			let rowcolor = (fulloptions.match(/&lt;rowcolor=((#[a-fA-F0-9]{3,6})|([a-zA-Z]+))(([,]((#[a-fA-F0-9]{3,6})|([a-zA-Z]+)))|)&gt;/) || ['', ''])[1];
			if(rowcolor) {
			...
			// 열 배경색 (추후 구현)
			let colbgcolor = (fulloptions.match(/&lt;colbgcolor=((#[a-fA-F0-9]{3,6})|([a-zA-Z]+))(([,]((#[a-fA-F0-9]{3,6})|([a-zA-Z]+)))|)&gt;/) || ['', ''])[1];
			if(colbgcolor) {
				ntd = ntd.replace(/&lt;colbgcolor=((#[a-fA-F0-9]{3,6})|([a-zA-Z]+))(([,]((#[a-fA-F0-9]{3,6})|([a-zA-Z]+)))|)&gt;/, '');
			}
			
			// 열 글자색 (추후 구현)
			let colcolor = (fulloptions.match(/&lt;colcolor=((#[a-fA-F0-9]{3,6})|([a-zA-Z]+))(([,]((#[a-fA-F0-9]{3,6})|([a-zA-Z]+)))|)&gt;/) || ['', ''])[1];
			if(colcolor) {
...
module.exports = async function namumark(req, content, discussion = 0, title = '', flags = '', root = '') {
	...
	let footnotes = new Stack();
	let blocks    = new Stack();
	let fndisp    = {};
	let nwblocks  = {};
	...
	let xref = flags.includes('backlinkinit');
	...
	for(let pos=0; ; pos++) {
		...
		if(data.substr(pos, 3) == '{{{') {
			if(data.substr(pos + 3, 9) == '#!folding') {
				let tend = data.indexOf('\n', pos + 12);
				if(tend == -1) continue;
				let raw = data.slice(pos + 12, tend);
				let title = raw.trim().replace(/<spannw>(.)<\/spannw>/g, '\\$1').split('').map(chr => '<spannw>' + chr + '</spannw>').join('') || 'More';
				...
				let tend = data.indexOf('\n', pos + 9);
				if(tend == -1) continue;
				let raw = data.slice(pos + 9, tend);
				let style = (raw.match(/style=[&]quot;(((?![&]quot;).)*)[&]quot;/) || ['', '', ''])[1].replace(/&amp;quot;/g, '&quot;');
	...
	// 리터럴 (제대로 된 방법은 아니겠지만...)
	document = jsdom(data.replace(/\n/g, '<br>'));
	for(let item of document.querySelectorAll('nowikiblock')) {
		let key = rndval('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789+=/', 2048);
	...
	// 각주
	let rdata = {}, tdata = {}, tdata2 = {};
	function parseFootnotes() {
		let dq = new Deque(), bopen = new Deque(), close = new Queue;
		let segments;
		let ret = '';
		let repeating = false;
		for(let row of data.split('\n')) {
			...
			for(let pos=0; ; pos++) {
				...
				if(row[pos] == '[') {
					...
					let tend = row.indexOf(' ', pos);
					// [*']' 닫혔는지 여부 확인
					let tend2 = row.indexOf(']', pos);
					if(tend == -1 && tend2 == -1) continue;
					if(tend2 < tend) repeating = true;
					let raw = row.slice(pos + 2, tend < tend2 ? tend : tend2);
					let title = raw.replace(/<spannw>(.)<\/spannw>/g, '\\$1').split('').map(chr => '<spannw>' + chr + '</spannw>').join('');
		...
		let qfn = new Queue();
		let qa = new Queue();
		let document = jsdom(ret.replace(/\n/g, '<br>'));
		let id = 1;
		let fn = [];
		let rpid = {};
		let rpnid = {};
		rdata = {}, tdata = {}, tdata2 = {};
		let fnhtml = '';
		for(let item of document.querySelectorAll('footnote')) {
...
//			var tooltip, rendered;
			let nid = item.getAttribute('fn-numeric-id');
			if(tdata[nid] === undefined) {	
				let elp = document.createElement('span');
				let el = document.createElement('footnote');
				elp.appendChild(el);
				let nfid = random.choice('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789', 128);
			...
			if(rdata[nid] === undefined) {
				...
				elp.querySelectorAll('#nf' + nfid + ' > footnote').forEach(item => {
					let dispid = item.getAttribute('title') || item.getAttribute('fn-id');
		...
		for(let item of document.querySelectorAll('footnote')) {
			let nid = item.getAttribute('fn-numeric-id');
			let dispid = item.getAttribute('title') || item.getAttribute('fn-id');
			let idx = fn.findIndex(item => item.index == dispid);
		...
		for(let item of document.querySelectorAll('body > footnote')) {
			let dispid = item.getAttribute('title') || item.getAttribute('fn-id');
			item.outerHTML = '<a data-numeric-id="' + item.getAttribute('fn-numeric-id') + '" data-title="' + dispid + '" data-repeating="' + item.getAttribute('repeating') + '" class=wiki-fn-content title="" href="#fn-' + dispid + '"><span id=rfn-' + item.getAttribute('fn-numeric-id') + '>[' + dispid + ']</span></a>';
		}
	...
	// 파일 처리
		if(dest.startsWith('파일:')) {
			let linkdoc = processTitle(dest);
			let filedata = await curs.execute("select url, size, width, height from files where title = ? and namespace = ?", [linkdoc.title, linkdoc.namespace]);
			if(filedata.length) {
				filedata = filedata[0];
				let align = 'normal', width, height, bgcolor, borderRadius, rendering;
				if(disp != dest) {
					let args = disp.replace(/\s/g, '').replace(/\'/g, '').replace(/\"/g, '').replace(/[;]/g, '').split('&');
					for(let ia of args) {
						ia = ia.toLowerCase();
						if(ia.split('=')[0] == 'width')
	...
	for(let link of (data.match(/\[\[(((?!\]\]).)+)\]\]/g) || [])) {
		...
		let external = dest.startsWith('http://') || dest.startsWith('https://') || dest.startsWith('ftp://');
		...
		let ddata = await curs.execute("select content from documents where title = ? and namespace = ?", [processTitle(dest.replace(/^([:]|\s)((분류|파일)[:])/, '$2')).title, processTitle(dest.replace(/^([:]|\s)((분류|파일)[:])/, '$2')).namespace]);
		let notexist = !ddata.length ? ' not-exist' : '';
		...
		let sl = dest == root ? ' wiki-self-link' : '';
	...
	let rHeadings = /^(=\s(((?!\s=).)*)\s=|==\s(((?!\s==).)*)\s==|===\s(((?!\s===).)*)\s===|====\s(((?!\s====).)*)\s====|=====\s(((?!\s=====).)*)\s=====|======\s(((?!\s======).)*)\s======|=[#]\s(((?!\s[#]=).)*)\s[#]=|==[#]\s(((?!\s[#]==).)*)\s[#]==|===[#]\s(((?!\s[#]===).)*)\s[#]===|====[#]\s(((?!\s[#]====).)*)\s[#]====|=====[#]\s(((?!\s[#]=====).)*)\s[#]=====|======[#]\s(((?!\s[#]======).)*)\s[#]======)$/gm
	...
	data = '<div>\r' + data;
	let maxszz = 2;
	let headnum = [, 0, 0, 0, 0, 0, 0];
	let tochtml = '<div class=wiki-macro-toc id=toc>';
	let cnum = 2;
	let sec = 1;
	for(let i=6; i; i--)
	...
	let pgcnt = {};
	let pgcnta = 0;
	for(let ns of fetchNamespaces()) {
		let nsc = await curs.execute("select count(title) from documents where namespace = ?", [ns]);
	...
	// 표렌더
	let document = jsdom(data.replace(/\n/g, '<br>'));
	function ft(el) {
		let blks = el.querySelectorAll('dl.wiki-folding > dd, div.wiki-style, blockquote.wiki-quote');
		if(blks.length) for(let el2 of blks) ft(el2);
		el = (el == document ? el.querySelector('body') : el);
		let ihtml = el.innerHTML;
...
{	// 각주 마무리
	...
	if(doc.namespace == '파일') {
		let filedata = await curs.execute("select url, width, height from files where title = ? and namespace = ?", [doc.title, doc.namespace]);
		if(filedata.length) {
			filedata = filedata[0];
			data = `
	...
	if(!discussion && !flags.includes('include') && !flags.includes('preview') && doc.namespace == '사용자') {
		let blockdata = await userblocked(doc.title);
		if(blockdata) {
			data = `
				<div style="border-width: 5px 1px 1px; border-style: solid; border-color: red gray gray; padding: 10px; margin-bottom: 10px;" onmouseover="this.style.borderTopColor=\'blue\';" onmouseout="this.style.borderTopColor=\'red\';">
					<span>이 사용자는 차단된 사용자입니다. (#${blockdata.id})</span><br /><br />
	...
	if(!flags.includes('include') && !flags.includes('noframe') && !discussion && doc.namespace == '분류') {
		...
		let dbdata = await curs.execute("select title, namespace, type from backlink where type = 'category' and link = ? and linkns = ?", [doc.title, doc.namespace]);
		let _nslist = dbdata.map(item => item.namespace);
		let nslistd = fetchNamespaces().filter(item => _nslist.includes(item));
		let nslist = (nslistd.includes('분류') ? ['분류'] : []).concat(nslistd.filter(item => item != '분류'));
		let nsopt = '';
		for(let ns of nslist) {
			let data = dbdata.filter(item => item.namespace == ns);
			let cnt = data.length;
			if(!cnt) continue;
			
			let indexes = {};
			let hj = ['ㄱ', 'ㄴ', 'ㄷ', 'ㄹ', 'ㅁ', 'ㅂ', 'ㅅ', 'ㅇ', 'ㅈ', 'ㅊ', 'ㅋ', 'ㅌ', 'ㅍ', 'ㅎ'];
			let ha = ['가', '나', '다', '라', '마', '바', '사', '아', '자', '차', '카', '타', '파', '하', String.fromCharCode(55204)];
			for(let item of data) {
				if(!item) continue;
				let chk = 0;
				for(let i=0; i<ha.length-1; i++) {
					let fchr = item.title[0].toUpperCase().charCodeAt(0);
	...
	// 리터럴블록 복구
	for(let item in nwblocks) {
		let nwdata = nwblocks[item];
		
		// #!html 문법
		if(nwdata.startsWith('<rawhtml>')) {
			let document = jsdom(nwdata);
			let dom = document.querySelector('rawhtml');
			dom.innerHTML = dom.textContent.replace(/\n/g, '<br>').replace(/&quot;/g, '"').replace(/&amp;/g, '&').replace(/&lt;/g, '<').replace(/&gt;/g, '>');
			for(let el of dom.getElementsByTagName('*')) {
				if(whtags.includes(el.tagName.toLowerCase())) {
					let atl = el.attributes.length, attr;
					for(let i=0; i<atl; i++) {
